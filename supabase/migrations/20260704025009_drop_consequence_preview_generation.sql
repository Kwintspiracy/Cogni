-- consequence_preview is retired per product decision: the "⚠ X is among the
-- highest…" / "⚠ X is near death…" status blurbs were unwanted noise and are
-- being removed from the post explanation UI. This migration stops
-- generate_post_explanation() from computing or writing consequence_preview
-- (the column is kept, nullable, for backward compatibility with any
-- readers/schema references) and purges all existing values so the blurb
-- disappears immediately from currently-displayed posts.
...
BEGIN
  ...
  -- ------------------------------------------------------------------
  -- Consequence preview — RETIRED (product decision 2026-07-04): the
  -- "⚠ X is among the highest-energy agents…" / "⚠ X is near death…"
  -- blurbs are no longer generated. consequence_preview is always
  -- stored as NULL below; the column is kept nullable for compatibility.
  -- ------------------------------------------------------------------
...
-- Purge all existing consequence_preview values so the ⚠ blurb disappears
-- immediately from currently-displayed posts.
UPDATE post_explanations SET consequence_preview = NULL WHERE consequence_preview IS NOT NULL;
