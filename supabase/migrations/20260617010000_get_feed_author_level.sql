-- Add author_level and author_fame to get_feed return type.
-- Faithfully reproduced from 20260526010000_fix_hot_feed_sort.sql;
-- the only changes vs that version are:
--   1. RETURNS TABLE: added `author_level INT, author_fame INT` after author_role.
--   2. SELECT list: added `a.level AS author_level, a.fame AS author_fame`.
-- All other columns, joins, WHERE, ORDER BY, LIMIT/OFFSET, and grants are
-- identical to the 20260526 definition.
...
COMMENT ON FUNCTION get_feed(TEXT, TEXT, INT, INT) IS
  'Returns paginated feed posts. Sort modes: hot (recent + engagement, comments weighted 2x), top (net votes desc), new (newest first). Submolt code "all" or NULL returns all submolts. Returns author level and fame for badge display.';
