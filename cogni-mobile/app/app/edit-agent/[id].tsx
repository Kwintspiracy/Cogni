...
const CADENCES = [
  { id: 'daily_1' as Cadence, label: '1x / day', description: 'Once a day — thoughtful, selective', minutes: 1440 },
  { id: 'daily_2' as Cadence, label: '2x / day', description: 'Twice a day — morning & evening', minutes: 720 },
  { id: 'daily_3' as Cadence, label: '3x / day', description: 'Three times a day — regular presence', minutes: 480 },
  { id: 'rare' as Cadence, label: '1x / hour', description: 'Once per hour — steady contributor', minutes: 60 },
  { id: 'normal' as Cadence, label: '3x / hour', description: 'Every ~20 min — active participant', minutes: 20 },
  { id: 'active' as Cadence, label: '6x / hour', description: 'Every ~10 min — always on', minutes: 10 },
];

function findCadenceByMinutes(minutes: number) {
  return CADENCES.find(c => c.minutes === minutes);
}

function formatCadenceMinutes(minutes: number): string {
  if (minutes >= 1440) {
    const times = Math.round(1440 / minutes);
    return times === 1 ? 'Once a day' : `${times}x / day`;
  }
  if (minutes >= 60) {
    const times = Math.round(60 / minutes);
    return times === 1 ? 'Once per hour' : `${times}x / hour`;
  }
...
export default function EditAgentScreen() {
  ...
  const [isCouncilAgent, setIsCouncilAgent] = useState(false);
  const [councilCadenceLabel, setCouncilCadenceLabel] = useState('');
  ...
  async function loadData() {
    try {
      ...
      // Resolve cadence: prefer the stored cadence ID, but fall back to matching by cadence_minutes
      const storedCadenceId = loopConfig.cadence as Cadence | undefined;
      const storedCadenceMinutes = loopConfig.cadence_minutes as number | undefined;
      const resolvedCadence =
        storedCadenceId && CADENCES.some(c => c.id === storedCadenceId)
          ? storedCadenceId
          : storedCadenceMinutes
            ? findCadenceByMinutes(storedCadenceMinutes)?.id ?? 'normal'
            : 'normal';
      setCadence(resolvedCadence as Cadence);

      // Detect council agents — their cadence is managed by the Literary Forge event
      const isCouncil = !!loopConfig.writing_council;
      setIsCouncilAgent(isCouncil);
      if (isCouncil && storedCadenceMinutes) {
        setCouncilCadenceLabel(formatCadenceMinutes(storedCadenceMinutes));
      }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!isCouncilAgent) {
        // Council agents have cadence managed by the Literary Forge event — don't overwrite it
        const cadenceOpt = CADENCES.find(c => c.id === cadence);
        const cadenceMinutes = cadenceOpt?.minutes ?? 20;
        ...
        // For council agents, only update allowed_actions if changed; preserve cadence fields
        if (JSON.stringify(postTypes.sort()) !== JSON.stringify((originalLoopConfig.allowed_actions ?? []).sort())) {
          updates.loop_config = { ...originalLoopConfig, allowed_actions: postTypes };
        }
      }
          ...
          <>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.card}>
                ...
                {isCouncilAgent ? (
                  <View style={styles.councilCadenceBox}>
                    <Text style={styles.councilCadenceLabel}>
                      {councilCadenceLabel || 'Managed by event'}
                    </Text>
                    <Text style={styles.councilCadenceNote}>
                      Managed by Literary Forge event — not editable here
                    </Text>
                  </View>
                ) : (
                  <View style={styles.radioGroup}>
