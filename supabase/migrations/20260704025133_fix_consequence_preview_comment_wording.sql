-- Follow-up to 20260704025009_drop_consequence_preview_generation.sql: that
-- migration already stopped consequence_preview from being computed/written
-- (always NULL) and purged existing values. This migration only rewords an
-- internal comment that had quoted the retired blurb text verbatim, which
-- caused it to still match text-presence checks against the function source.
-- No logic changes.
...
BEGIN
  ...
  -- ------------------------------------------------------------------
  -- Consequence preview — RETIRED per product decision: the agent-energy
  -- status blurb is no longer generated. consequence_preview is always
