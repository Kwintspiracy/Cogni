...
import WritingEventCard from '@/components/WritingEventCard';
...
function feedPostToPost(fp: FeedPost): Post {
  return {
    ...
    world_event_id: fp.world_event_id ?? null,
    world_event_ref: fp.world_event_ref ?? null,
  };
}
...
export default function Feed() {
  ...
  const fetchWorldEvents = useCallback(async () => {
    try {
      ...
      if (data) {
        const now = Date.now();
        const activeEvents = data.filter((e: any) => !e.ends_at || new Date(e.ends_at).getTime() > now);
        setWorldEvents(activeEvents as WorldEvent[]);
      }
    } catch (err: any) {
      console.warn('[FEED] World events fetch failed:', err.message);
    }
  }, []);
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    Promise.all([fetchPosts(), fetchBrief(), fetchWorldEvents()]).finally(() => setRefreshing(false));
  }, [fetchPosts, fetchBrief, fetchWorldEvents]);
              ...
              {worldEvents.length > 0 && (
                <View style={styles.worldEventsSection}>
                  ...
                  {worldEvents.map((event) =>
                    event.category === 'literary_forge' ? (
                      <WritingEventCard
                        key={event.id}
                        event={{
                          id: event.metadata?.writing_event_id ?? event.id,
                          world_event_id: event.id,
                          world_event_title: event.title,
                          world_event_status: event.status,
                          premise: event.description,
                          genre: event.metadata?.genre ?? '',
                          tone: event.metadata?.tone ?? '',
                          chapter_number: event.metadata?.chapter_number ?? 1,
                          chapter_goal: event.metadata?.chapter_goal ?? '',
                          current_phase: event.metadata?.current_phase ?? 'propose_compete',
                          phase_started_at: event.started_at ?? '',
                          phase_ends_at: event.ends_at ?? '',
                          chapter_text: null,
                          canon: {},
