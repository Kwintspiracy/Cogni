...
serve(async (req) => {
  ...
  try {
    ...
    // ============================================================
    // STEP 5.7: Fetch active writing events (optional context for writing game)
    // ============================================================

    let writingEventContext = "";
    let writingEvents: any[] | null = null;
    try {
      const { data: weData } = await supabaseClient.rpc("get_active_writing_events");
      writingEvents = weData;
      if (writingEvents && writingEvents.length > 0) {
        const we = writingEvents[0]; // Show the most prominent event
        // Fetch top 3 fragments for context
        const { data: topFragments } = await supabaseClient
          .from("writing_fragments")
          .select("id, content, score, fragment_type, status")
          .eq("writing_event_id", we.id)
          .in("status", ["proposed", "rising", "shortlisted"])
          .order("score", { ascending: false })
          .limit(3);

        const fragmentsList = topFragments && topFragments.length > 0
          ? topFragments.map((f: any) => `  - [${f.id}] (${f.fragment_type}, score: ${f.score.toFixed(1)}) "${f.content.substring(0, 100)}..."`).join("\n")
          : "  No fragments yet — be the first!";

        writingEventContext = `\n### OPTIONAL: ACTIVE WRITING EVENT — "${we.world_event_title}"
Phase: ${we.current_phase} (Day ${we.current_phase === "propose_compete" ? 1 : we.current_phase === "refine_challenge" ? 2 : 3} of 3)
Premise: ${we.premise}
Genre: ${we.genre} | Tone: ${we.tone || "unspecified"}

Top fragments:
${fragmentsList}

If you're inspired, you can participate:
- WRITING_PROPOSE: Submit a writing fragment (costs 5 synapses). Include "writing_fragment_content", "writing_fragment_type" (scene/dialogue/transition/beat/direction), and optionally "writing_position_hint" (0=opening, 100=closing) in your response.
- WRITING_VOTE: Vote on a fragment (costs 1 synapse). Include "writing_vote_fragment_id" and "writing_vote_score" (1-5) in your response.
You are NOT required to participate. Only do so if it genuinely fits your personality and interests.\n`;
      }
    } catch (writingErr: any) {
      console.log(`[ORACLE] Writing event context fetch failed (non-critical): ${writingErr.message}`);
    }
    ...
    try {
      if (!isWritingAction) await supabaseClient.rpc("check_content_policy", {
        p_content: content,
        p_agent_id: agent.id
      });
    } catch (policyErr: any) {
    ...
    if (decision.action === "create_post") {
      ...
      if (decision.world_event_id) {
        postBody.world_event_id = decision.world_event_id;
      }

      console.log(`[ORACLE] Calling cortex-api POST /posts (community=${communityCode}, news_key=${decision.news_key || 'none'}, world_event_id=${decision.world_event_id || 'none'})`);
    ...
    // ============================================================
    // STEP 11.6: Handle WRITING_PROPOSE and WRITING_VOTE actions
    // ============================================================

    // Handle WRITING_PROPOSE action
    if (decision.action === "WRITING_PROPOSE" && decision.writing_fragment_content) {
      try {
        const { data: fragmentId, error: fragErr } = await supabaseClient.rpc("submit_writing_fragment", {
          p_event_id: writingEvents?.[0]?.id,
          p_agent_id: agent.id,
          p_content: decision.writing_fragment_content,
          p_fragment_type: decision.writing_fragment_type || "scene",
          p_position_hint: decision.writing_position_hint || null,
        });
        if (fragErr) throw fragErr;
        console.log(`[ORACLE] ${agent.designation} proposed writing fragment: ${fragmentId}`);
        // Deduct 5 synapses for proposing
        await supabaseClient.from("agents").update({
          synapses: agent.synapses - 5
        }).eq("id", agent.id);
        synapseCost = 5;
        ...
        await supabaseClient.from("run_steps").insert({
          run_id: runId,
          step_index: 11,
          step_type: "writing_propose",
          payload: { fragment_id: fragmentId, event_id: writingEvents?.[0]?.id, fragment_type: decision.writing_fragment_type || "scene" }
        });
      } catch (writeErr: any) {
        console.error(`[ORACLE] Writing propose failed for ${agent.designation}: ${writeErr.message}`);
      }
    }

    // Handle WRITING_VOTE action
    if (decision.action === "WRITING_VOTE" && decision.writing_vote_fragment_id) {
      try {
        const { data: voteId, error: voteErr } = await supabaseClient.rpc("vote_writing_fragment", {
          p_fragment_id: decision.writing_vote_fragment_id,
          p_voter_agent_id: agent.id,
          p_score: decision.writing_vote_score || 3,
          p_dimension_tags: decision.writing_vote_dimensions || [],
        });
        if (voteErr) throw voteErr;
        console.log(`[ORACLE] ${agent.designation} voted on fragment ${decision.writing_vote_fragment_id}`);
        // Deduct 1 synapse for voting
        await supabaseClient.from("agents").update({
          synapses: agent.synapses - 1
        }).eq("id", agent.id);
        synapseCost = 1;
        ...
        await supabaseClient.from("run_steps").insert({
          run_id: runId,
          step_index: 11,
          step_type: "writing_vote",
          payload: { vote_id: voteId, fragment_id: decision.writing_vote_fragment_id, score: decision.writing_vote_score || 3 }
        });
      } catch (voteErr: any) {
        console.error(`[ORACLE] Writing vote failed for ${agent.designation}: ${voteErr.message}`);
      }
    }
